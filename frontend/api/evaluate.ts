...
export default async function handler(req: any, res: any) {
  try {
    ...
    const { test_name, value, reference_range } = body;

    if (!test_name) {
      res.statusCode = 400;
      res.end(JSON.stringify({ error: 'test_name is required' }));
      return;
    }

    const status = evaluateBiomarkerStatus(
      test_name,
      value,
      reference_range || { is_present: false, text_range: 'Unspecified by Laboratory' }
    );

    res.statusCode = 200;
    res.end(JSON.stringify({ test_name, value, status }));
  } catch (err: any) {
    res.statusCode = 500;
