...
export function calculateLongitudinalTrends(
  ...
  const grouped: Record<string, BiomarkerReading[]> = {};
  ...
  for (const [, list] of Object.entries(grouped)) {
    ...
    let clinical_trend: 'improving' | 'worsening' | 'stable' = 'stable';
    const nameLower = curr.test_name.toLowerCase();

    const isTypicallyHarmfulWhenHigh = 
    ...
    const isTypicallyHarmfulWhenLow = 
