...
export function parseClinicalTextOffline(rawText: string): LocalExtractionResult {
  const lines = rawText.split(/\r?\n/);
  const readings: BiomarkerReading[] = [];
  const extractedMeds: { name: string; dosage?: string; frequency?: string }[] = [];
  let extractedPatientInfo: LocalExtractionResult['extracted_patient_info'] = {};
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const catalogEntry of TEST_CATALOG) {
      if (processedTests.has(catalogEntry.canonicalName)) continue;
      ...
      const status: BiomarkerStatus = evaluateBiomarkerStatus(catalogEntry.canonicalName, numVal, refRange);
  ...
  const summary = `Local Clinical Parser extracted ${readings.length} structured biomarkers. ${
    outOfRange.length > 0 
      ? `${outOfRange.length} value(s) fall outside source laboratory reference intervals.`
      : 'All extracted values fall within reported reference intervals.'
  } Zero-hallucination reference range rules strictly enforced.`;
