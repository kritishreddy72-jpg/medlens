 ...
 */
export function detectClinicalConflicts(patient, readings, extractedMedications) {
    const conflicts = [];
    ...
    for (const allergy of patient.allergies) {
        ...
        for (const drugClass of DRUG_CLASSES) {
            const isAllergicToClass = allergyLower.includes(drugClass.class_name.toLowerCase()) ||
            ...
            if (isAllergicToClass) {
                ...
                for (const med of allMeds) {
                    const matchesMed = drugClass.generic_names.some(g => med.includes(g)) ||
    ...
    if (potassiumReading && typeof potassiumReading.value === 'number' && potassiumReading.value >= 5.2) {
        const isTakingRaasInhibitor = allMeds.some(m => /lisinopril|enalapril|ramipril|losartan|valsartan|candesartan|spironolactone|eplerenone/i.test(m));
    ...
    if (egfrReading && typeof egfrReading.value === 'number' && egfrReading.value < 45) {
        const isTakingNsaid = allMeds.some(m => /ibuprofen|naproxen|meloxicam|diclofenac|ketorolac|celecoxib|advil|aleve/i.test(m));
        if (isTakingNsaid) {
            conflicts.push({
    ...
    if (plateletReading && typeof plateletReading.value === 'number' && plateletReading.value < 60) {
        const isTakingAnticoag = allMeds.some(m => /warfarin|coumadin|apixaban|eliquis|rivaroxaban|xarelto|clopidogrel|plavix|dabigatran/i.test(m));
        if (isTakingAnticoag) {
            conflicts.push({
